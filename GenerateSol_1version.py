...
def Phase1_2(r):
    ...
    print('shippment', Total_shippment_cost)
    print("cost_inventory", cost_inventory)
    print("cost stock", cost_stockout)
    return TOTAL_COST, risk, times
...
def Phase3(B = 1e100):
    ...
    for i in range(len(elite_sol)):
        ...
        cost, risk, _ = Phase1_2(elite_sol[i])
        cost1, risk1, _1 = Phase1_2(INIT_r[i])
        print("elite")
        print(cost)
        print("INIT")
        print(cost1)
...
def local_search(sol):
    ...
    while  m < M:
        ...
        col = random.randint(0, len(T) - 1)
        newSol[row][col] = a
        _, current_risk, _ = Phase1_2(newSol)
        if current_risk < new_risk:
            bestsol = newSol
            new_cost, new_risk, _ = Phase1_2(bestsol)
...
def worstSol(eliteSol):
    ...
    worstSol = None
    for i in eliteSol:
        current_cost, risk, _ = Phase1_2(i)
        if risk > new_risk:
            worstSol = i
            new_risk = risk
...
# BestSol = VNS()
# TotalCost, TotalRisk, Times = Phase1_2(BestSol)
