...
def local_search(sol):
    ...
    row = random.randint(0, len(RC) - 1)
    col = random.randint(0, len(T) - 1)
    while new_cost > current_cost and m < M:
        a = random.random()
        newSol[row][col] = a
        new_cost = Phase1_2(newSol)
        m += 1
...
def VNS():
    ...
    while k < k_max and iter <= maxRuns:
        ...
        elif Phase1_2(newSol) < Phase1_2(worstSol(elite_sol)):
            print("ADD to elite solution due to substitution")
            elite_sol.remove(worstSol(elite_sol))
            elite_sol.append(newSol)
            iter += 1
            print("lenth", len(elite_sol))
            print(elite_sol)

        if Phase1_2(newSol) < Phase1_2(base_sol):
            base_sol = newSol
            k = 1
        else:
            print("k+=1")
